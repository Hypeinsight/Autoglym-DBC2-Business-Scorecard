  ...
  return (
    ...
    >
      <div className="mb-2.5 flex items-start justify-between gap-2">
        <div className="text-[0.68rem] font-bold uppercase tracking-[0.07em] text-muted">
          {metric.name}
        </div>
        {info && <InfoTooltip what={info.what} source={info.source} label={`What "${metric.name}" measures`} />}
      </div>
      ...
      {editingTarget === 'headline' ? (
        <div className="mb-1 flex items-center gap-1.5">
          <input
            ...
            disabled={isSaving}
            className="figure w-full rounded-md border border-brand px-2 py-1 font-display text-[1.6rem] font-extrabold leading-none text-ink outline-none focus:ring-2 focus:ring-brand/30"
          />
          {metric.unit && <span className="text-[0.85rem] font-medium text-muted">{metric.unit}</span>}
        </div>
      ) : (
        <div
          className={`figure mb-1 font-display text-[1.6rem] font-extrabold leading-none tracking-tight text-ink ${canEditHeadline ? 'cursor-pointer decoration-brand/40 decoration-2 underline-offset-4 hover:underline' : ''}`}
